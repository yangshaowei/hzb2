...
import com.cbs.okserver.task.PriorityAsyncTask;
...
public class DownloadTask extends PriorityAsyncTask<Void, DownloadInfo, DownloadInfo> {
    ...
    protected DownloadInfo doInBackground(Void... params) {
        ...
        if (TextUtils.isEmpty(fileName)) {//设置文件名
            //fileName = HttpUtils.getNetFileName(response, url);//根据http头或url得到文件名
            fileName=mDownloadInfo.getTaskKey();//以taskKey作为文件名
