package com.cbs.Constant;

/**
 * Created by yangshaowei on 2017/5/8.
 */

public class AppConstant {

    public static final String invite = "/invite_link";
    public static final String GetSign = "/sign";
    public static final String Update = "/modify_bill";
    public static final String SearchAll = "/find_all_bill";
    public static final String CreatBill = "/add_bill";
    public static final String Login = "/login";
    public static final String Register = "/register";
    public static final String PORT = ":5001";
    public static final String BASEURL = "http://45.32.129.172";

    public static final String CreatBillUrl = BASEURL+PORT+CreatBill;
    public static final String RegisterUrl = BASEURL+PORT+Register;
    public static final String LoginUrl = BASEURL+PORT+Login;
    public static final String SearchAllUrl = BASEURL+PORT+SearchAll;
    public static final String UpdateBillUrl = BASEURL+PORT+Update;
    public static final String GetSignUrl = BASEURL+PORT+GetSign;
    public static final String InviteUrl = BASEURL+PORT+invite;
}
